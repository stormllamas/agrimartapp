...
const Cart = ({
  ...
  const proceedToPayments = async () => {
    ...
    if(currentOrder.count < 1 && deliveryAddressId === '' && !delivery && !lastName && !firstName && !contact && !email ? false : true) {
      const formData = {
        ...
        // description,
        promoCode: promoCodeSet ? promoCodeRetrieved.id : false
      }
      await checkout({
        formData,
  ...
  const addressSelected = async () => {
    ...
    let perKmTotal = Math.round((parseInt(res.data.distanceValue)/1000)*siteInfo.per_km_price)
      ...
      !currentOrderLoading && currentOrder !== null && (
        currentOrder.order_items.length > 0 ? (
          <Animated.View style={[styles.superModal, { top: modalAnim }]}>
            <TopNavigation
              accessoryLeft={() => <TopNavigationAction onPress={() => setCartActive(false)} icon={props => <Icon {...props} name='arrow-back'/>}/>}
              title={`My Cart`}
            />
            <ScrollView>
              ...
              <View style={[styles.collapsibleWrapper, { backgroundColor: 'white', padding: 10 }]}>
                ...
                {currentOrder.order_items !== undefined && (
                  currentOrder.order_items.map(orderItem => (
                    <View key={orderItem.id} style={[styles.orderItem, (!orderItem.final_stock > 0 ? {backgroundColor: '#EEEEEE', color: '#606060'} : {})]}>
                      ...
                      <View style={{ paddingRight: 50 }}>
                        ...
                        <View style={styles.productQuantity}>
                          <TouchableOpacity
                            style={[styles.decreaseQuantity, (orderItem.quantity > 1 && orderItem.final_stock > 0 ? {} : {backgroundColor: '#EEEEEE'})]}
                            onPress={orderItem.quantity > 1 && !quantityLoading && orderItem.product.is_published ? (() => changeQuantity({ orderItemID: orderItem.id, operation: 'subtract' })) : undefined}
                          >
                          ...
                          <TouchableOpacity
                            style={[styles.increaseQuantity, (orderItem.quantity < 10  && orderItem.quantity < orderItem.final_stock&& orderItem.quantity < orderItem.final_stock ? {} : {backgroundColor: '#EEEEEE'})]}
                            onPress={orderItem.quantity < 10 && orderItem.quantity < orderItem.final_stock && !quantityLoading && orderItem.product.is_published ? (() => changeQuantity({ orderItemID: orderItem.id, operation: 'add' })) : undefined}
                          >
                            <Ionicons style={orderItem.quantity < 10 && orderItem.quantity < orderItem.final_stock && orderItem.product.is_published ? { color: '#4CAF50' } : {color: '#606060'}} name="chevron-forward-outline" size={16}/>
                          </TouchableOpacity>
                        </View>
                        <Text style={[styles.mute, { fontSize: 14 }]}>{orderItem.quantity} x ₱ {orderItem.product_variant.final_price.toFixed(2)}</Text>
                        <Text>₱ {orderItem.total_price.toFixed(2)}</Text>
                      </View>
                      <Ionicons style={styles.deleteOrderItem} name="trash-outline" size={20} onPress={() => deleteOrderItem({ id:orderItem.id })}/>
                    </View>
                  ))
                )}
              </View>
              ...
              <View style={[styles.collapsibleWrapper]}>
                ...
                <View style={[styles.collapsibleContent, {marginBottom: 100, borderWidth: 0}]}>
                  <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 15 }}>
                    ...
                    <Text>
                      <Text style={[styles.mute, styles.strikethrough]}>{promoCodeSet && (getPromoCode().final_order_discount*100 === 0 ? '' : (`₱${Math.round(currentOrder.subtotal)} `))}</Text>
                      {currentOrder.subtotal ? `₱${promoCodeSet && getPromoCode().final_order_discount > 0 ? Math.round(currentOrder.subtotal*((1-getPromoCode().final_order_discount))-0.1).toFixed(2) : currentOrder.subtotal.toFixed(2)}` : '₱0.00' }
                    </Text>
                  </View>
                  <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 15 }}>
                    <Text style={[styles.mute, { fontSize: 14 }]}>Delivery</Text>
                    <Text>
                      <Text style={[styles.mute, styles.strikethrough]}>{promoCodeSet && ((getPromoCode().final_delivery_discount*100) === 100 ? 'FREE': getPromoCode().final_delivery_discount*100 === 0 ? '' : (`₱${Math.round(delivery/(1-getPromoCode().final_delivery_discount)).toFixed(2)} `))}</Text>
                  ...
                  <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginVertical: 15 }}>
                    ...
                    <Text style={{ fontFamily: 'Lato-Black', fontSize: 20 }}>
                      {currentOrder.count < 1 || deliveryAddressId === '' || !delivery || !lastName || !firstName || !contact || !email ? '' : `₱${(((promoCodeSet && getPromoCode().final_order_discount > 0) ? Math.round((currentOrder.subtotal*((1-getPromoCode().final_order_discount).toFixed(2)))-0.1) : currentOrder.subtotal)+parseInt(delivery)).toFixed(2)}`}
                    </Text>
                  </View>
                </View>
              </View>
...
export default connect(mapStateToProps, { getCurrentOrder, deleteOrderItem, getAddress, changeQuantity, checkout })(Cart);