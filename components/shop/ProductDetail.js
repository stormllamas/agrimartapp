  ...
  return (
    !productLoading && rendered ? (
      product !== null && productImages.length > 0 ? (
        <>
          <Header subtitle='Food' backLink={{component:'Root', options: {screen: 'Products' }}} navigation={navigation}/>
          ...
          {isAuthenticated && !currentOrderLoading && currentOrder !== null && currentOrder.order_items.length > 0 ? (
            <Ionicons style={[styles.foodCartButton, {bottom: 60}]} name="cart" size={28} color={"#ffffff"} onPress={() => setCartActive(!cartActive)}/>
          ) : undefined}
          <Cart seller={product.seller} cartActive={cartActive} setCartActive={setCartActive} navigation={navigation} />
        </>
      ) : (
        <>
          <Text>Not Found</Text>
